...
export default async function NewCoachingProgramPage() {
  let user: User;

  try {
    user = await requireUser();
  } catch {
    redirect(`/auth/login?callbackUrl=/admin/coaching/programs/new`);
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          Add a coaching program such as Matric Coaching or Entry Test Preparation.
        </p>
      </div>

      <CoachingProgramForm mode="create" />
    </div>
  );
}
