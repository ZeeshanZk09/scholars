...
export default async function AdminBlogTagsListPage() {
  let user: User;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-lg font-semibold text-slate-900">Blog Tags</h1>
          <p className="mt-1 text-sm text-slate-600">
            Keywords used to label and filter blog posts.
          </p>
        </div>
        {canCreate ? (
          <Link
            href="/admin/blogs/tags/new"
            ...
            {tags.length === 0 ? (
              ...
              tags.map((tag) => (
                <tr key={tag.id}>
                  ...
                  <td className="hidden px-4 py-3 text-slate-500 md:table-cell">{tag.slug}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      {canUpdate ? (
                        <Link
                          href={`/admin/blogs/tags/${tag.id}`}
                      ...
                      {canDelete ? (
                        <DeleteButton
                          id={tag.id}
                          endpoint="/api/v1/admin/blogs/tags"
                          label="Delete"
                        />
                      ) : null}
                    </div>
